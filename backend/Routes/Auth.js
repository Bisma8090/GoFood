...
router.post('/orderData', async (req, res) => {
  try {
    let order_data = req.body.order_data;  // frontend se items
    let email = req.body.email;

    console.log("📩 Order Request from:", email);

    // ✅ Validation: har order object me Order_date aur items array exist karte hain ya nahi
    for (let order of order_data) {
      if (!order.Order_date || !order.items || !Array.isArray(order.items)) {
        return res.status(400).json({ success: false, message: "Invalid order format" });
      }
    }

    // Pehle check karo ke email ka order already exist karta hai ya nahi
    let existingOrder = await Order.findOne({ email: email });

    if (!existingOrder) {
      // Agar pehli dafa order kar raha hai
      ...
      // Agar pehle se order exist karta hai, concat use karke add karo
      existingOrder.order_data = existingOrder.order_data.concat(order_data);
